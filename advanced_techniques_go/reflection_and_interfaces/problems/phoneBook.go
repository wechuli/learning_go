package problems // Make necessary changes in the code below:
import (
	"encoding/csv"
	"errors"
	"fmt"
...
func saveCSVFile(filepath string) error {
	...
	if currentFormat == "entry" {
		...
		for _, row := range data2 {
			temp := []string{row.Name, row.Surname, row.Areacode, row.Tel, row.LastAccess}
			_ = csvwriter.Write(temp)
		}
	}
