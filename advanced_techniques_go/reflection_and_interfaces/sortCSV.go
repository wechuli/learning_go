...
func readCSVFile(filepath string) error {
	...
	for _, line := range lines {
		...
		if format1 {
			temp := F1{Name: line[0], Surname: line[1], Tel: line[2], LastAccess: line[3]}
			d1 = append(d1, temp)
		} else {
			temp := F2{Name: line[0], Surname: line[1], Areacode: line[2], Tel: line[3], LastAccess: line[4]}
			d2 = append(d2, temp)
		}
	}
...
// First based on the surname. If they have the same surname take into account the name
...
// First based on areacode. If they have the same areacpde take into account the surnmae
